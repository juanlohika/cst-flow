/**
 * Pilot Tracker — apply a roster XLSX (commits to DB).
 *
 * POST /api/accounts/[id]/pilot-tracker/import/apply
 *   multipart/form-data with `file` (.xlsx)
 *
 * Re-parses the file server-side (doesn't trust the client to echo the
 * validation report back) and upserts participants for all `ok`+`warn`
 * rows. Writes a PilotUploadBatch audit record.
 */
...
import { validateWorkbook, applyValidated } from "@/lib/pilot/bulk-import";
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      warnRows: report.warnRows,
      errorRows: report.errorRows,
    });
  } catch (e: any) {
    return NextResponse.json(
      { error: e?.message || "Failed to apply XLSX" },
      { status: 500 },
    );
  }
}
