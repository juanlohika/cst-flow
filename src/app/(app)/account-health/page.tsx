...
function Content() {
  ...
  // Any signed-in user can view — backend scopes accounts to their memberships.
  ...
  if (!canView) return <div className="max-w-3xl mx-auto p-8"><p className="text-rose-700 font-bold">Please sign in to view Account Health.</p></div>;
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-4">
      ...
      {loading && !summary ? (
        ...
        <>
          ...
          {/* All accounts table */}
          {(() => {
            const totalAccts = summary.accounts.length;
            const totalPages = Math.max(1, Math.ceil(totalAccts / ACCOUNTS_PER_PAGE));
            const currentPage = Math.min(accountsPage, totalPages);
            const startIdx = (currentPage - 1) * ACCOUNTS_PER_PAGE;
            const pageRows = summary.accounts.slice(startIdx, startIdx + ACCOUNTS_PER_PAGE);
            return (
              <section className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
                <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between flex-wrap gap-2">
                  <h2 className="text-[13px] font-black text-slate-800">All accounts</h2>
                  <p className="text-[11px] text-slate-500">
                    {totalAccts === 0 ? "0 accounts" : `Showing ${startIdx + 1}–${Math.min(startIdx + ACCOUNTS_PER_PAGE, totalAccts)} of ${totalAccts}`}
                  </p>
                </div>
                ...
                {totalPages > 1 && (
                  <div className="px-5 py-3 border-t border-slate-100 flex items-center justify-between gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg border border-slate-200 text-[11px] font-black uppercase tracking-widest text-slate-600 disabled:opacity-40 disabled:cursor-not-allowed hover:border-indigo-300"
                    >
                      ← Prev
                    </button>
                      ...
                      {Array.from({ length: totalPages }).map((_, i) => {
                        ...
                        if (totalPages <= 7 || p === 1 || p === totalPages || (p >= currentPage - 1 && p <= currentPage + 1)) {
                          return (
                            <button
                              key={p}
                              onClick={() => setAccountsPage(p)}
                              className={`min-w-[28px] h-7 px-2 rounded-md text-[11px] font-bold transition-colors ${p === currentPage ? "bg-indigo-500 text-white" : "text-slate-600 hover:bg-slate-100"}`}
                            >
                              {p}
                            </button>
                          );
                        } else if (p === currentPage - 2 || p === currentPage + 2) {
                          return <span key={p} className="text-slate-300 text-[11px]">…</span>;
                        }
                        return null;
                      })}
                    </div>
