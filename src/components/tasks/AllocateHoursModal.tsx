...
export default function AllocateHoursModal({ projectId, parentTask, onClose, onSuccess }: AllocateHoursModalProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      showToast(`${rows.length} subtask${rows.length > 1 ? "s" : ""} created`, "success");
      onSuccess();
    } catch (err: any) {
      showToast(err.message || "Failed to create subtasks", "error");
    } finally {
      setSaving(false);
    }
  };
  ...
  const AssigneeSelect = ({ value, onChange }: { value: string; onChange: (v: string) => void }) => (
    <select
      ...
      className="w-full border border-slate-100 rounded-lg px-2 py-1.5 text-[10px] font-bold text-slate-600 bg-white outline-none focus:ring-2 focus:ring-primary appearance-none"
    >
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-xl rounded-2xl shadow-xl border border-slate-200 overflow-hidden flex flex-col max-h-[85vh]">
        ...
        {/* Footer */}
        <div className="p-3 border-t bg-slate-50/20 flex items-center justify-between shrink-0">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-5 py-2 bg-primary text-white rounded-md text-[10px] font-bold uppercase tracking-widest hover:bg-primary/90 disabled:opacity-50 transition-all shadow shadow-primary/20"
          >
            {saving ? <Loader2 className="w-3 h-3 animate-spin" /> : <CheckCircle className="w-3 h-3" />}
            {saving ? "Creating..." : `Create ${rows.length} Sub-task${rows.length !== 1 ? "s" : ""}`}
          </button>
        </div>
      </div>
    </div>
